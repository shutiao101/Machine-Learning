...
class PCAcomponent(object):
    def __init__(self, X, N=3):
        ...
        self.N = N
        self.variance_ratio = []
        self.low_dataMat = []  # 降维后的数据矩阵

    def _fit(self):
        X_mean = np.mean(self.X, axis=0)
        dataMat = self.X - X_mean  # 数据中心化
        covMat = np.cov(dataMat, rowvar=False)
        # or covMat = dataMat.T * dataMat / dataMat.shape[0] 手搓协方差矩阵
        # 若rowvar非0，一列代表一个样本；为0，一行代表一个样本
        eigVal, eigVect = np.linalg.eig(np.mat(covMat))
        # 求特征值[矩阵形式]和特征向量，特征向量是按列放的，即一列代表一个特征向量，同时对应一个特征值
        eigValInd = np.argsort(eigVal)  # Index 从小到大排序
        eigValInd = eigValInd[-1:-(self.N + 1):-1]  # 取前N个较大的特征值索引,切片（从后往前取）
